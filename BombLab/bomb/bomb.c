...
int main(int argc, char *argv[])
{
     ...
     * 于是要凑一个 0x9 0xf 0xe 0x5 0x6 0x7 为 ASCII 尾数的字符串就行了。因为我们知道 A 的 ASCII 码是 0x41，
     * 所以很容易推算出一个序列：IONEFG 当然，这个结果也不唯一，只是用字母比较方便。
     * */
    phase_defused();
     ...
     * 首先 read_six_numbers(), 从原栈区取出 read_six_numbers 获取的第一个整数，
     * 要求这个整数必须小于等于 6u (这段生成的代码很奇怪，先减 1 再跟 5 比)。
     * 然后是将 r12d++, 接着将 r12d 跟 6 比较，猜测是循环。
     * 接着将 r12d 作为索引，4 为 stride，将 (%rsp,%rax,4) 拷贝到 eax 里面，其实也就是刚才读进来的整数。
     * 然后比较 (%rbp) 和 %eax, 如果相等就 explode 了。但是个人觉得这个应该没办法相等吧。
     * 然后又是一个循环。于是这看起来就像是一个类似于：
     * for (int i = 1; i != 6; i++) {
     *     for (int j = i; j <= 5; j++) {
     *         // ...
     *     }
     ...
     * 这样的一个 nested loop.
     * 
     * 接着又是一个循环，循环变量是一个指针，作用是将输入的 6 个数字，每一个都**被 7 减**。
     * 完成后，在数字串中找 1，如果没找到就结束，如果找到了就循环。
     * 将 0x6032d0 加载到了栈中，没太看懂这到底是加载到了什么地方。
     *
     * 实在是看不懂了，参考了网上的答案。原来是一个链表。
     * (gdb) x/3x 0x6032d0
     * 0x6032d0 <node1>:	0x0000014c	0x00000001	0x006032e0
     *                         node value      node number     pointer to next node
     * (gdb) x/3x 0x006032e0
     ...
     *
     * 然后将值从大到小排序后，对应的序号序列就是答案：3 4 5 6 1 2
     * 不过我参考的那个答案好像是老版本的，这个答案填进去是不对的。这里我想到了上面读到了一段所有数字被 7 减的代码，
     * 就尝试了 4 3 2 1 6 5，顺利通过了。
     *
     * 很难，这个算完成了一半吧。
     * */
    phase_defused();
    ...
    /*
     * 这里已经在疯狂暗示了，其实早在做 phase_2 的时候我就发现了还有个 secret_phase。
     * 需要手动去 call 这个函数才能调用。难度太高了，不做了。
     * */
